...
window.showCadastro = (usuario) => {
  ...
  document.getElementById("conteudo").innerHTML = `
    ...
    <input id="telefone" placeholder="Telefone (55DDD+Número)" />
...
window.cadastrar = async (usuario) => {
  ...
  if (telefone && telefone.length >= 12) {
    const msg = `Olá ${cliente}, aqui é da Buck Doces. Sua compra foi registrada no valor de R$ ${valor.toFixed(2)}. Obrigado pela preferência! 🍬`;
    const url = `https://wa.me/${telefone}?text=${encodeURIComponent(msg)}`;
    window.open(url, "_blank");
  }
};

// As funções showDashboard e showCobranca abaixo são completas e seguem o padrão original:
